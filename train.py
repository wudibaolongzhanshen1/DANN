...
config_path = 'config/2024-06-03-14-30-36/config.txt'
config_dict = {}
...
with open('dataset\\mnist_data.pkl', 'rb') as f:
    ...
    mnist_train_dataset = MnistmDataset(mnist_train_imgs, mnist_train_labels, transform=mnistm_train_transforms)
    mnist_test_dataset = MnistmDataset(mnist_test_imgs, mnist_test_labels, transform=mnistm_train_transforms)
    mnist_train_loader = DataLoader(dataset=mnist_train_dataset, batch_size=batch_size, shuffle=True)
    mnist_test_loader = DataLoader(dataset=mnist_test_dataset, batch_size=batch_size, shuffle=True)
    mnist_train_num = len(mnist_train_dataset)
    mnist_test_num = len(mnist_test_dataset)
...
for i in range(epoch):
    ...
    for step in range(len_dataloader):
        net.zero_grad()
        mnist_imgs, mnist_labels = datasource_iter.__next__()
        mnistm_imgs, mnistm_labels = datatarget_iter.__next__()
        ...
        source_domain_loss = SourceDloss(source_domain_pred_results, torch.zeros_like(source_domain_pred_results))
        target_domain_loss = TargetDloss(target_domain_pred_results, torch.ones_like(target_domain_pred_results))
